...
PLANE_CKPT="${PLANE_CKPT:-logs/FixedStageFv3/20260523_152906-fixedFv3-locomotion-hunter/model_3500.pt}"
...
"$PY" humanoidverse/train_agent.py \
  ...
  ++rewards.reward_scales.termination=-10.0 \
  ...
  project_name=FixedStageFv4 experiment_name=fixedFv4 \
  2>&1 | tee "$LOG_ROOT/train.log"

echo "[fixedF] --- POST-PPO eval at each ckpt ---"
for ck in "$LOG_ROOT"/*fixedFv4-locomotion-hunter/model_*.pt; do
  [ -f "$ck" ] || continue
